...
Measured on Apple-M1 (MacBookAir10,1), macOS 26.5.2, hevc-4k-hdr10.mkv, windowed, render surface not recorded by this session, 60 s, median of 2.
...
- **AetherEngine**: 1439/1438 delivered/expected, dropped 0, 3840x1714, 10-bit, hdr10, rendered into render surface not reported.
- **KSPlayer**: 1440/1440 delivered/expected, dropped 0, 3840x1714, 10-bit, SMPTE_ST_2084_PQ, rendered into render surface not reported. Served via **KSMEPlayer**.
- **VLCKit**: 1454/1440 delivered/expected, dropped 0, 3840x1714, bit depth and color transfer not reported by this engine, rendered into render surface not reported.
- **libmpv**: 1443/1440 delivered/expected, dropped 0, 3840x1714, 10-bit, pq, rendered into render surface not reported.
...
**Caveat on the GPU column in this session.** The four engines hosted in an AppKit window rendered into 3840x2160 pixels while libmpv rendered into 1920x1080, because the window size was requested in points for the former and in pixels for the latter. That was measured after the fact, not during the run, so libmpv did roughly a quarter of the pixel work of the engines it is compared against here and its figure is correspondingly favourable. The benchmark now records the surface each engine actually rendered into and refuses to present the GPU column as comparable when they differ; this table predates that.
