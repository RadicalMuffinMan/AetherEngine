...
- **Two subtitle cues sharing a PTS no longer overwrite one another in the packet store.** Insertion assumed at most one packet per timestamp, so anything landing at an already-occupied PTS replaced the entry there, on the premise that a repeat PTS could only mean the pump and the forward-prefetcher re-harvesting the same packet. That premise doesn't hold for ASS/SSA: overlapping lines are routinely authored with identical Start/End, and simultaneous karaoke syllables or duplicate glow styles are simply distinct packets that happen to carry the same timestamp, so the second write silently discarded the first cue instead of storing both. The insert now scans forward across every entry at that PTS and only collapses one whose payload is byte-identical to the incoming packet, the actual signature of a re-harvest. Not just a timestamp match, but with `bytes` adjusted against the entry it replaces. Anything else at the same PTS is inserted alongside its neighbors rather than over them.
