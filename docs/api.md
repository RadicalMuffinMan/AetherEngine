...
One kind carries a number a host will want to read: `.sourceRefused` means the origin answered the source request with an HTTP status instead of media, and `underlyingCode` is that status (a 401/403 refusal — on a connection-capped IPTV panel most often "the slot is still held" —, a 404, a 5xx). It is distinct from `.sourceOpenFailed`, which is what a corrupt or unreadable source produces; before it existed the two arrived alike as "Invalid data found when processing input".
