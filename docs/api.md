...
| `$playbackPhase` | The derived one-source-of-truth status: adds `.rebuffering` and `.stalled(reconnecting:)`. Prefer it over stitching `state` + `isBuffering` + `isSeeking`, and over matching log text. `.rebuffering` is published on every path, including the bare-AVPlayer audio host (a starved progressive stream after the item has played), where `.stalled` cannot occur because there is no reader. |
