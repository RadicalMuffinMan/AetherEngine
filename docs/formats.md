...
- H.264 High 4:2:2 / 4:4:4 / High-10 and HEVC Rext (4:2:2 / 4:4:4 / 12-bit) on hardware whose VideoToolbox has no decoder for the profile (Intel Macs, older Apple TV chips). AVPlayer accepts these at the HLS CODECS level and the item reaches `readyToPlay`, but the native path then renders nothing, so a per-format `VTDecompressionSession` probe at load (`VTCapabilityProbe.canHardwareDecode`) routes them to libavcodec, which decodes them (#2). Apple Silicon has HW decoders for all of these and keeps them native. The probe only judges a config record that actually carries out-of-band parameter sets: a source with in-band VPS/SPS/PPS (`hev1`, `numOfArrays = 0`) gives VideoToolbox no SPS to build a session from, which says nothing about hardware support, so those keep the native path (AetherPlayer#2).
...
**Wireless AirPlay (#86, #227).** While an iOS session plays to a wireless AirPlay receiver, the engine reloads its loopback over the device's LAN IP: the receiver fetches the stream for itself and cannot reach `127.0.0.1`. An SDR source keeps the master there, so its `SUBTITLES` renditions travel and `setNativeSubtitleSelected(track:)` has a legible group to select against; the `EXT-X-MEDIA` URIs are relative, so the renditions resolve against the LAN base. An HDR / DV source is handed the media playlist instead and its subtitles do not travel. `$nativeSubtitleRenditionsServed` reports which of the two happened, so a host can tell the user rather than silently dropping their subtitles. A wired HDMI external display is a different route and keeps the loopback plus its master.

That HDR limit is a property of the receiver, measured rather than assumed (2026-07-27, iPhone 17 Pro to two different Apple TV 4K units, both in HDR mode). The receiver takes an SDR master, including 4K-capable HEVC, and refuses every master describing HDR content while decoding those same PQ segments happily off the media playlist. Dropping the DV `SUPPLEMENTAL-CODECS`, clamping the declared `BANDWIDTH`, omitting `RESOLUTION` and declaring `HDCP-LEVEL=TYPE-1` each changed nothing, which leaves `VIDEO-RANGE=PQ` itself, and declaring the range as SDR over HDR content was already disproven in #98 Stage 1.5 (the compatibility gate reads the real `colr` and codec). A `SUBTITLES` rendition can only live in a master, so for HDR / DV the media playlist is not a workaround but the only routable manifest.

The refusal is silent: no `-11868`, no failed item, the rate flickers to `playing` for a tick so `hasEverPlayed` latches, and the picture simply never starts. A progress watchdog therefore backs the hop: eight seconds without the clock advancing on a master handed to a receiver reloads the LAN media playlist, so a receiver that cannot take even the SDR master (no 4K HEVC, say) recovers into playback instead of parking on AVKit's "not playable on this display" sign.
...
// true while the served playlist carries the SUBTITLES group; goes false on a
// media-playlist fallback, on the wireless-AirPlay hop for an HDR / DV source, and
// when the receiver refuses the master it was handed (#227). Hosts use it to decide
// whether to draw their own subtitle window on a wired external display instead
// (#98), or to tell the user that subtitles will not travel to this route
